    ...
    <div class="bg-base-100 rounded-lg shadow p-6 relative">
        ...
        <form method="POST" action="{{ isset($step) ? route('newsletter-editor.update', $step->id) : route('newsletter-editor.store') }}">
            ...
            <div class="flex justify-end space-x-1">
                @if(isset($step))
                    <a href="/preview/newsletter/{{ $step->order }}" class="btn btn-info" target="_blank">👁️ View</a>
    ...
    <div class="mt-6 p-4 bg-base-200 rounded-lg shadow">
       <pre>
        Newsletter Variables:
                $record->name  - Full name (first + last)
                $record->first  - First name only
                $record->last  - Last name only
                $record->email  - Email address
                $firstName  - First name (alias)
                $lastName  - Last name (alias)
                $email  - Email address (alias)
                $currentStep  - Current step number
                $unsubscribeUrl  - Unsubscribe link
                $record->tour_date  - Date field
                $record->tour_date_str  - String field
                $daysToGo - number days
        </pre>
    </div>
</div>
...
function selectImage(url, alt) {
    console.log('Selecting image:', url, alt); // Debug log
    if (window.imageCallbackFn) {
        // Call the TinyMCE callback with proper parameters
