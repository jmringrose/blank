...
@section('content')
    <div class="container mx-auto px-4 py-2">
        <div class="bg-base-100 rounded-lg shadow p-2 relative">
            <a href="{{ $cancelRoute }}" class="btn btn-info absolute top-4 right-4">Cancel</a>
            <div class="mb-6">
                @if(isset($step))
                    <div class="mb-4 flex p-2">
                        <h1 class="text-2xl font-bold">{{ $step->title }}</h1>
                        <span class="text-stone-200 ml-6 font-semibold"><span class="material-symbols-outlined">file_open</span> {{ $step->filename }}</span>
                        <span class="text-stone-200 ml-6 font-semibold">{{ $stepLabel }}: #{{ $step->order }}</span>
                    </div>
                @else
            ...
            <form method="POST" action="{{ isset($step) ? $updateRoute : $storeRoute }}">
                ...
                <div class="flex justify-end space-x-3">
                    @if(isset($step) && isset($previewRoute))
                        <a href="{{ $cancelRoute }}" class="btn btn-info btn-sm float-left">Cancel</a>
                        <a href="{{ $previewRoute }}" class="btn btn-info btn-sm" target="_blank"><span class="material-symbols-outlined">mystery</span>View</a>
                    @endif
                    @if(isset($step))
                        <button type="button" onclick="saveAndContinue()" class="btn btn-info btn-sm">Save & Continue</button>
                        <button type="submit" name="action" value="save" class="btn btn-info btn-sm">Save {{ $type }} Email</button>
                    @else
    ...
    @push('scripts')
        <script>
            ...
            function showToast(message, type) {
                ...
                toast.style.cssText = 'position: fixed; top: 20px; right: 20px; padding: 12px 20px; border-radius: 6px; color: white; font-weight: 500; z-index: 9999; background: ' + (type === 'success' ? '#10b981' : '#ef4444');
